...
│   ├── scrape_data.sh   # Скрапинг данных из sitemap.xml
...
├── scraper.go           # Go-загрузчик для sitemap.xml (НОВОЕ!)
...
#### Способ 1: Ручное добавление файлов
...
#### Способ 2: Автоматический скрапинг через sitemap.xml
```bash
# Загрузка данных из sitemap.xml
./scripts/scrape_data.sh https://example.com/sitemap.xml

# Импорт скачанных данных
./scripts/import_data.sh
```

**Возможности Go-загрузчика:**
- 🌐 **Автоматический парсинг** sitemap.xml
- 📄 **Извлечение контента** из HTML (заголовок, описание, основной текст)
- 💾 **Сохранение в MD формате** с правильной структурой
- 🔄 **Резервное копирование** существующих данных
- ⚡ **Контроль скорости** загрузки с задержками

**Системные требования для скрапера:**
- **Go 1.21+** для компиляции и запуска загрузчика
...
# 🌐 Скрапинг данных из sitemap.xml
./scripts/scrape_data.sh https://example.com/sitemap.xml
